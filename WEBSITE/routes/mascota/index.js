...
// AQUI VA EL CODIGO SOLICITADO PARA EL TRABAJO DEL EXAMEN
...
router.post('/solicitar-servicio',cors(),async(req,res,next)=>{
  ...
  var mascota=req.body;
  result= await db.solicitarServicio(req.body);
...
router.post('/insertar-articulo',cors(),async(req,res,next)=>{
  console.log("insertar articulo")
  var result={};
  console.log("params", req.body);

  var mascota=req.body;
  result= await db.insertarArticulo(mascota);
...
//Listar ventas de la tienda de mascota
router.get('/obtener-lista-ventas', cors(), async (req, res, next) => {
